...
export function PaperCard({
  ...
  const faviconOrgs = (orgs || []).filter((o) => o.favicon);
  const visibleFavicons = faviconOrgs.slice(0, 5);
  const extraOrgs = (orgs?.length || 0) - visibleFavicons.length;
  const statusSymbol =
  ...
  return (
            ...
            {orgs && orgs.length > 0 && (
              <div className="mt-2">
                ...
                >
                  ...
                  {extraOrgs > 0 && (
                    <span className="text-[11px] text-zinc-300">+ {extraOrgs}</span>
                  )}
                </button>
                {showOrgs && (
    ...
    {showFull && (
      <div className="fixed inset-0 z-50 bg-black/60 flex justify-end overflow-y-auto">
        <motion.div
          initial={{ x: "100%" }}
          animate={{ x: 0 }}
          exit={{ x: "100%" }}
          transition={{ type: "spring", stiffness: 260, damping: 30 }}
          className="h-full w-full max-w-md bg-slate-950 p-6 overflow-y-auto max-h-[100dvh]"
        >
          <button
