...
export async function fetchAuthorTextFromArxivHtml(
  ...
  const res = await fetch(url, {
    headers: {
      "User-Agent": "arxiv-authors-only/0.2",
  ...
  const prefix = new TextDecoder("utf-8").decode(buf);
...
async function wikidataEntity(qid: string): Promise<Record<string, unknown>> {
  const params = new URLSearchParams({
...
function firstClaimValue(entity: Record<string, unknown>, prop: string) {
...
function domainFromP856(entity: Record<string, unknown>): string | null {
...
export async function orgToFavicon(name: string, size = 128): Promise<OrgInfo> {
  ...
  const entity = qid ? await wikidataEntity(qid) : {};
